...
function rSel(){
  ...
  ps.forEach(function(p){
    ...
    h+='<section><button type="button" aria-current="'+(sel?'true':'false')+'" title="'+E(statusTitleProject(p))+'" class="project-link group w-full text-left text-txt-300 hover:text-txt-100 transition-colors" data-project="'+E(p.id)+'" onclick="selectProject(this.dataset.project)">'+
      '<div class="flex items-center gap-2 min-w-0"><span class="w-[5px] h-[5px] rounded-full '+pss.dot+(pss.label==='working'?' pulse':'')+' shrink-0"></span><span class="font-mono font-semibold truncate">'+E(p.name||p.path||p.id)+'</span>'+chip(pss.label,pss.color)+'</div>'+
      '<div class="mt-1 ml-3 text-[10px] text-txt-500">'+active+' team'+(active!==1?'s':'')+'</div></button>';
    h+='<div class="mt-2 ml-[7px] border-l border-base-800/80">';
    teams.sort((a,b)=>b.timeUpdated-a.timeUpdated).forEach(function(t){const ss=coarseTeamStatus(t),tsel=c&&c.id===t.id;h+='<button type="button" title="'+E(statusTitleTeam(t))+'" class="team-link block w-full text-left border-l-2 border-transparent -ml-px py-1.5 pl-3 pr-2 text-[11px] text-txt-500 hover:text-txt-200 hover:bg-base-900/70 transition-colors" aria-current="'+(tsel?'true':'false')+'" data-team="'+E(t.id)+'" onclick="selectTeam(this.dataset.team)"><div class="flex items-center gap-2 min-w-0"><span class="w-[5px] h-[5px] rounded-full '+ss.dot+(ss.label==='working'?' pulse':'')+' shrink-0"></span><span class="truncate font-mono">'+E(t.name)+'</span><span class="ml-auto text-[9px] uppercase tracking-wide text-txt-500">'+E(ss.label)+'</span></div></button>'});
    h+='</div></section>';
  });
  h+='</div></nav>';
  patch(el,h);
...
function rSum(t){
  ...
  patch(el,
    ...
    (t.projectId?chip(E(t.projectId),'muted'):'')+
